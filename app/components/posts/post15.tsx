import {
   PostSection,
   ParagraphContainer,
   TitleContainer,
   PhraseContainer,
   Phrase,
   Author,
   MainTitle,
   H2,
   H3,
   H4,
   H5,
   Paragraph,
   Separator,
   Break,
} from './posts-styles';

export default function Post15() {
   return (
      <>
         <PostSection id="page-19">
            <TitleContainer>
               <H4 className="big">41-42°C</H4>
            </TitleContainer>

            <Separator />
            <Separator />
            <Separator className="mobile" />

            <PhraseContainer>
               <Phrase>
                  voy a exagerar
                  <Break />
                  mi fiebre no es tan alta
               </Phrase>
               <Author>-OKTUBRE</Author>
            </PhraseContainer>

            <Separator />
            <Separator />
            <Separator className="mobile" />

            <ParagraphContainer className="">
               <Paragraph className="align-justify">
                  mi primer pucho me lo convidó un perso
                  <span className="large space-xb">timbre timbre</span>
                  <Break />
                  vivía enfrente al cementerio
                  <span className="large space-xb">
                     me escondí en el counter
                  </span>
                  <Break />
                  jamás le respondí si me siento solo a veces
                  <span className="large space-xxl">y bajé a abrir</span>
                  <Break />
                  ni le voy a devolver los casets de <em>Okupas</em>
                  <span className="large space-m">era el del correo</span>
                  <Break />
                  me lo ofreció en un gesto doloroso, vivo
                  <span className="large space-l">
                     <em>-¿está tu mamá?</em>
                  </span>
                  <Break />
                  fue extender hacia mí
                  <span className="large space-l">
                     <em>-no</em> -le dije y me dejó un paquete
                  </span>
                  <Break />
                  su camel diez y sólo cuando dije sí
                  <span className="large space-xl">obvio que lo abrí ahí</span>
                  <Break />
                  el índice abrió la caja el pulgar lo deslizó
                  <span className="large space-xl">seguro un terro</span>
                  <Break />
                  me dio el encendedor, que lo encienda yo
                  <span className="large space-xxb">ya me mató</span>
                  <Break />
                  tal como dijo que pasaría: tosí, un poco
                  <span className="large space-g">con cuchillo</span>
                  <Break />
                  él sonrío entre orgullo, goce y malicia
                  <span className="large space-xl">era un consolador</span>
                  <Break />
                  me dijo <em>vamos a pedir algo, tengo una lija</em>
                  <span className="large space-xxb">y una carta</span>
                  <Break />
                  me mareé y él sonrió con triste expresión
                  <span className="large space-xl">supuestamente</span>
                  <Break />
                  caminábamos, hizo entonces su pregunta
                  <span className="large space-xxb"> de la actual</span>
                  <Break />
                  él se prendió otro y yo acepté otro, pero
                  <span className="large space-b"> mujer de papá</span>
                  <Break />
                  me dijo <em>tranqui, juntá ganas, disfrutale</em>
                  <span className="large space-xb">pero era obvio</span>
                  <Break />
                  yendo a buscar las burguers, me lo prendí
                  <span className="large space-xb"> que era ella</span>
                  <Break />
                  él prendió otro; lo miró y, apenas, me dijo
                  <span className="large space-xl"> estaba escrita</span>
                  <Break />
                  <em>loco vos sabés que éste es mi compañero</em>
                  <span className="large space-xl"> tan cruelmente</span>
               </Paragraph>
            </ParagraphContainer>

            <ParagraphContainer className="on-left-side desktop">
               <Paragraph className="align-left">
                  mi primer pucho me lo convidó un perso
                  <Break />
                  vivía enfrente al cementerio
                  <Break />
                  jamás le respondí si me siento solo a veces
                  <Break />
                  ni le voy a devolver los casets de <em>Okupas</em>
                  <Break />
                  me lo ofreció en un gesto doloroso, vivo
                  <Break />
                  fue extender hacia mí
                  <Break />
                  su camel diez y sólo cuando dije sí
                  <Break />
                  el índice abrió la caja el pulgar lo deslizó
                  <Break />
                  me dio el encendedor, que lo encienda yo
                  <Break />
                  tal como dijo que pasaría: tosí, un poco
                  <Break />
                  él sonrío entre orgullo, goce y malicia
                  <Break />
                  me dijo <em>vamos a pedir algo, tengo una lija</em>
                  <Break />
                  me mareé y él sonrió con triste expresión
                  <Break />
                  caminábamos, hizo entonces su pregunta
                  <Break />
                  él se prendió otro y yo acepté otro, pero
                  <Break />
                  me dijo <em>tranqui, juntá ganas, disfrutale</em>
                  <Break />
                  yendo a buscar las burguers, me lo prendí
                  <Break />
                  él prendió otro; lo miró y, apenas, me dijo
                  <Break />
                  <em>loco vos sabés que éste es mi compañero</em>
               </Paragraph>

               <Paragraph className="align-right">
                  <span className="large">timbre timbre</span>
                  <Break />
                  <span className="large">me escondí en el counter</span>
                  <Break />
                  <span className="large">y bajé a abrir</span>
                  <Break />
                  <span className="large">era el del correo</span>
                  <Break />
                  <span className="large">
                     <em>-¿está tu mamá?</em>
                  </span>
                  <Break />
                  <span className="large">
                     <em>-no</em> -le dije y me dejó un paquete
                  </span>
                  <Break />
                  <span className="large">obvio que lo abrí ahí</span>
                  <Break />
                  <span className="large">seguro un terro</span>
                  <Break />
                  <span className="large">ya me mató</span>
                  <Break />
                  <span className="large">con cuchillo</span>
                  <Break />
                  <span className="large">era un consolador</span>
                  <Break />
                  <span className="large">y una carta</span>
                  <Break />
                  <span className="large">supuestamente</span>
                  <Break />
                  <span className="large">de la actual</span>
                  <Break />
                  <span className="large">mujer de papá</span>
                  <Break />
                  <span className="large">pero era obvio</span>
                  <Break />
                  <span className="large">que era ella</span>
                  <Break />
                  <span className="large">estaba escrita</span>
                  <Break />
                  <span className="large">tan cruelmente</span>
               </Paragraph>
            </ParagraphContainer>

            <Separator />
            <Separator />
            <Separator />
            <Separator className="mobile" />
         </PostSection>
      </>
   );
}
